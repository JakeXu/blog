...
const Projects = ({ projectsData }: Props) => {
  ...
  const projects = useCreation(() => {
    return JSON.parse(projectsData) as matter.GrayMatterFile<string>[]
  }, [projectsData])
    ...
    <section id="projects">
      ...
      <StyledProjectsGrid>
        {projects?.map(({ data, content }, i) => {
          const { external, title, tech, github, cover, cta } = data

          return (
            <StyledProject
              key={i}
              ref={el => {
                if (el) revealProjects.current[i] = el
              }}
            >
              <div className="project-content">
                <div>
                  ...
                  <div className="project-links">
                    ...
                    {external && !cta && (
                      <a target="_blank" rel="noreferrer" href={external} aria-label="External Link" className="external">
                        <Icon name="External" />
                      </a>
                    )}
                  </div>
              ...
              <div className="project-image">
                <a target="_blank" rel="noreferrer" href={external ? external : github ? github : '#'}>
                  <Image fill src={cover} quality={25} alt={title} className="img" />
                </a>
              </div>
            </StyledProject>
          )
